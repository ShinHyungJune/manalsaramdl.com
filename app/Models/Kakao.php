...
class Kakao extends Model
{
    ...
    public function getMessage($to, $data, $template)
    {
        ...
        if($template == KakaoTemplate::ORDER_SUCCESS){
            $message = array(
                "#{merchant_uid}" => $data["merchant_uid"], // order id
        ...
        if($template == KakaoTemplate::ANSWER_REGISTER){
            $message = [
                "#{url}" => $data["url"],
            ];
        }

        if($template == KakaoTemplate::PASSWORD_RESET){
            $message = [
                "#{url}" => $data["url"],
            ];
        }

        if($template == KakaoTemplate::NUMBER_SEND){
            $message = [
                "#{number}" => $data["number"]
            ];
        }
